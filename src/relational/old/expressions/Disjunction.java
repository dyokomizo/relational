...
public class Disjunction<R extends Relation> implements Term<R, Boolean>,
	...
	public static <R extends Relation> Term<R, Boolean> or(
		...
		final List<Term<R, Boolean>> list = new ArrayList<Term<R, Boolean>>(
				terms);
		return new Disjunction<R>(Collections.unmodifiableList(list));
	}
	...
	private Disjunction(List<Term<R, Boolean>> terms) {
		super();
		this.terms = terms;
	}
	...
	public R relation() {
		return terms.get(0).relation();
	}

	public Term.Precedence precedence() {
		return Term.Precedence.DISJUNCTION;
	}
}